...
Options:
    --pad-to-ref    Pad code to reference code_length using reference bytes
    --verify        Verify checksums of existing image
"""
...
# MS2109 default header (from device)
# [0x00-0x01] Magic: 0xA5 0x5A
# [0x02-0x03] Code length (uint16 BE)
# [0x04]      Hook flags (0x05 = normal + IRQ hooks)
# [0x05]      Config (0x10)
# [0x06-0x0B] Reserved (0xFF)
# [0x0C-0x0F] USB config (0x20 0x20 0x07 0x07)
# [0x10-0x2F] USB strings (0xFF = use ROM defaults)
# Checksum: sum(0x02:0x30), no skip bytes

DEFAULT_HEADER = bytearray([
    0xA5, 0x5A,         # magic
    0x00, 0x00,         # code length (filled in)
    0x05, 0x10,         # hook flags, config
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,  # reserved
    0x20, 0x20, 0x07, 0x07,  # USB config
